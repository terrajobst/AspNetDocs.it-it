...
---
# <a name="aspnet-signalr-hubs-api-guide---net-client-signalr-1x"></a>Guida dell'API Hub SignalR ASP.NET-client .NET (SignalR 1. x)
...
- [Guida dell'API degli hub SignalR-client JavaScript](../guide-to-the-api/hubs-api-guide-javascript-client.md)
...
L'oggetto proxy è thread-safe. Infatti, se si chiama `HubConnection.CreateHubProxy` più volte con lo stesso `hubName`, si ottiene lo stesso oggetto `IHubProxy` memorizzato nella cache.
...
Per altre informazioni, vedere informazioni [e gestione degli eventi di durata della connessione in SignalR](../guide-to-the-api/handling-connection-lifetime-events.md).
