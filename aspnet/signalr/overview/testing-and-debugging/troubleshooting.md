...
- [Configurazione di WebSocket IIS in ping/pong per rilevare un client non attivo](#pong)
...
### <a name="ondisconnect-not-firing-at-consistent-times"></a>Disconnect non viene attivato in momenti coerenti

Questo comportamento è previsto dalla progettazione. Quando un utente tenta di uscire da una pagina con una connessione SignalR attiva, il client SignalR effettuerà il tentativo di notificare al server che la connessione client verrà arrestata. Se il tentativo del client di SignalR non riesce a raggiungere il server, il server eliminerà la connessione dopo un `DisconnectTimeout` configurabile in un secondo momento, a quel punto il `OnDisconnected` evento verrà attivato. Se il tentativo del client SignalR ha esito positivo, l'evento `OnDisconnected` viene attivato immediatamente.

Per informazioni sull'impostazione dell'impostazione di `DisconnectTimeout`, vedere [gestione degli eventi di durata della connessione: DisconnectTimeout](../guide-to-the-api/handling-connection-lifetime-events.md#disconnecttimeout).
...
## <a name="configuring-iis-websockets-to-pingpong-to-detect-a-dead-client"></a>Configurazione di WebSocket IIS in ping/pong per rilevare un client non attivo

I server SignalR non sanno se il client è inattivo o meno e si basano sulla notifica dal WebSocket sottostante per gli errori di connessione, ovvero il callback `OnClose`. Una soluzione a questo problema consiste nel configurare i WebSocket di IIS per eseguire il ping/pong. In questo modo si assicura che la connessione venga chiusa in caso di interruzione imprevista. Per altre informazioni, vedere [questo post di StackOverflow](http://stackoverflow.com/questions/19502755/websocket-clients-state-not-changing-on-network-loss).
...
- **Aggiunta di route all'applicazione prima di aggiungere la route dell'hub:** Se l'applicazione usa altre route, verificare che la prima route aggiunta sia la chiamata a `MapSignalR`.
- **Utilizzando IIS 7 o 7,5 senza l'aggiornamento degli URL senza estensione:** L'uso di IIS 7 o 7,5 richiede un aggiornamento per gli URL senza estensione, in modo che il server possa fornire l'accesso alle definizioni dell'hub in `/signalr/hubs`. L'aggiornamento è disponibile [qui](https://support.microsoft.com/kb/980368).
- **Cache IIS non aggiornata o danneggiata:** Per verificare che il contenuto della cache non sia aggiornato, immettere il comando seguente in una finestra di PowerShell per cancellare la cache:
...
Questo errore si verifica in genere anche se un firewall o un proxy non è configurato correttamente, causando la riscrittura delle intestazioni della richiesta. La soluzione consiste nel verificare che la porta 80 sia abilitata sul firewall o sul proxy.

### <a name="unexpected-response-code-500"></a>"Codice di risposta imprevisto: 500"

Questo errore può verificarsi se la versione di .NET Framework utilizzata nell'applicazione non corrisponde alla versione specificata in Web. config. La soluzione consiste nel verificare che .NET 4,5 venga utilizzato sia nelle impostazioni dell'applicazione sia nel file Web. config.

### <a name="typeerror-lthubtypegt-is-undefined-error"></a>Errore "TypeError: &lt;hubType&gt; non definito"

Questo errore viene generato se la chiamata a `MapSignalR` non viene eseguita correttamente. Per ulteriori informazioni, vedere [come registrare il middleware SignalR e configurare le opzioni di SignalR](../guide-to-the-api/hubs-api-guide-server.md#route) .
...
### <a name="invalidoperationexception-not-a-valid-web-socket-request"></a>"InvalidOperationException: non è una richiesta di socket Web valida.

Questo errore può verificarsi se si usa il protocollo WebSocket, ma il proxy di rete modifica le intestazioni della richiesta. La soluzione consiste nel configurare il proxy per consentire WebSocket sulla porta 80.

### <a name="exception-ltmethod-namegt-method-could-not-be-resolved-when-client-calls-method-on-server"></a>"Eccezione: non è stato possibile risolvere il nome del metodo &lt;&gt; metodo" quando il client chiama il metodo sul server

Questo errore può derivare dall'uso di tipi di dati che non possono essere individuati in un payload JSON, ad esempio Array. La soluzione alternativa consiste nell'usare un tipo di dati individuabile da JSON, ad esempio IList. Per altre informazioni, vedere il [client .NET non è in grado di chiamare i metodi dell'hub con parametri di matrice](https://github.com/SignalR/SignalR/issues/2672).
...
Questo errore viene visualizzato se `MapSignalR` viene chiamato due volte dall'applicazione. Alcune applicazioni di esempio chiamano `MapSignalR` direttamente nella classe Startup; altri effettuano la chiamata in una classe wrapper. Assicurarsi che l'applicazione non esegua entrambe le operazioni.

### <a name="websocket-is-not-used"></a>WebSocket non è usato

Se è stato verificato che il server e i client soddisfino i requisiti per WebSocket (elencati nel documento [piattaforme supportate](../getting-started/supported-platforms.md) ), è necessario abilitare WebSocket sul server. Le istruzioni per eseguire questa operazione sono disponibili [qui](https://www.iis.net/learn/get-started/whats-new-in-iis-8/iis-80-websocket-protocol-support).

### <a name="connection-is-undefined"></a>$. la connessione non è definita

Questo errore indica che gli script in una pagina non vengono caricati correttamente oppure che il proxy dell'hub non è raggiungibile o che l'accesso non è corretto. Verificare che i riferimenti agli script nella pagina corrispondano agli script caricati nel progetto e che sia possibile accedere a/SignalR/hubs in un browser quando il server è in esecuzione.
...
Questo errore indica che manca la libreria `Microsoft.CSharp`. Aggiungerlo nella scheda **assembly-&gt;Framework** .

### <a name="caller-state-cannot-be-accessed-from-clientscaller-in-visual-basic-or-in-a-strongly-typed-hub-conversion-from-type-taskof-object-to-type-string-is-not-valid-error"></a>Non è possibile accedere allo stato del chiamante da client. chiamante in Visual Basic o in un hub fortemente tipizzato; Errore "conversione dal tipo ' (Of Object)' al tipo ' String ' non valido

Per accedere allo stato del chiamante in Visual Basic o in un hub fortemente tipizzato, usare la proprietà `Clients.CallerState` (introdotta in SignalR 2,1) invece di `Clients.Caller`.
...
Questo comportamento è previsto dalla progettazione. SignalR richiede .NET Framework 4 o versione successiva. Questa operazione richiede che le applicazioni SignalR siano sviluppate in Visual Studio 2010 o versioni successive. Il componente server di SignalR richiede .NET Framework 4,5.
...
SignalR è supportato in IIS 7,0 e 7,5, ma è necessario aggiungere il supporto per gli URL senza estensione. Per aggiungere il supporto per gli URL con estensione, vedere [https://support.microsoft.com/kb/980368](https://support.microsoft.com/kb/980368)

SignalR richiede l'installazione di ASP.NET nel server (per impostazione predefinita, ASP.NET non è installato in IIS). Per installare ASP.NET, vedere [download di ASP.NET](https://www.asp.net/downloads).
...
Gli argomenti usati dal backplane di Azure vengono gestiti internamente; non sono progettate per essere configurabili dall'utente.
