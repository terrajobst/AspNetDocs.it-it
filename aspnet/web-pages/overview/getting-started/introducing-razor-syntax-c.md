...
>   
> 
> <span data-ttu-id="e1ac9-115">Questa esercitazione funziona anche con Pagine Web ASP.NET 2.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-115">This tutorial also works with ASP.NET Web Pages 2.</span></span>
...
<span data-ttu-id="e1ac9-244">Nei blocchi di codice server è spesso necessario restituire testo o markup (o entrambi) alla pagina.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-244">In server code blocks, you often want to output text or markup (or both) to the page.</span></span> <span data-ttu-id="e1ac9-245">Se un blocco di codice server contiene testo che non è codice e che invece deve essere sottoposto a rendering così come sono, ASP.NET deve essere in grado di distinguere il testo dal codice.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-245">If a server code block contains text that's not code and that instead should be rendered as is, ASP.NET needs to be able to distinguish that text from code.</span></span> <span data-ttu-id="e1ac9-246">Esistono diversi modi per eseguire tale operazione.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-246">There are several ways to do this.</span></span>
...
<span data-ttu-id="e1ac9-283">Per il markup, come probabilmente si saprà, è possibile creare un commento HTML:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-283">For markup, as you probably know, you can create an HTML comment:</span></span>
...
<span data-ttu-id="e1ac9-370">Ecco un esempio che consente di comprendere le differenze:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-370">Here's an example to help you understand the differences:</span></span>
...
## <a name="conditional-logic-and-loops"></a><span data-ttu-id="e1ac9-401">Logica condizionale e cicli</span><span class="sxs-lookup"><span data-stu-id="e1ac9-401">Conditional Logic and Loops</span></span>
...
<span data-ttu-id="e1ac9-410">In questo esempio, se la prima condizione nel blocco If non è true, viene verificata la condizione `else if`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-410">In this example, if the first condition in the if block is not true, the `else if` condition is checked.</span></span> <span data-ttu-id="e1ac9-411">Se tale condizione viene soddisfatta, vengono eseguite le istruzioni nel blocco `else if`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-411">If that condition is met, the statements in the `else if` block are executed.</span></span> <span data-ttu-id="e1ac9-412">Se nessuna delle condizioni viene soddisfatta, vengono eseguite le istruzioni nel blocco `else`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-412">If none of the conditions are met, the statements in the `else` block are executed.</span></span> <span data-ttu-id="e1ac9-413">È possibile aggiungere qualsiasi numero di altri blocchi if, quindi chiudere con un blocco di `else` come &quot;tutto il resto&quot; condizione.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-413">You can add any number of else if blocks, and then close with an `else` block as the &quot;everything else&quot; condition.</span></span>

<span data-ttu-id="e1ac9-414">Per testare un numero elevato di condizioni, usare un blocco `switch`:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-414">To test a large number of conditions, use a `switch` block:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample52.cshtml)]

<span data-ttu-id="e1ac9-415">Il valore da testare è racchiuso tra parentesi (nell'esempio, la variabile di `weekday`).</span><span class="sxs-lookup"><span data-stu-id="e1ac9-415">The value to test is in parentheses (in the example, the `weekday` variable).</span></span> <span data-ttu-id="e1ac9-416">Ogni singolo test usa un'istruzione `case` che termina con i due punti (:).</span><span class="sxs-lookup"><span data-stu-id="e1ac9-416">Each individual test uses a `case` statement that ends with a colon (:).</span></span> <span data-ttu-id="e1ac9-417">Se il valore di un'istruzione `case` corrisponde al valore di test, viene eseguito il codice in quel blocco case.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-417">If the value of a `case` statement matches the test value, the code in that case block is executed.</span></span> <span data-ttu-id="e1ac9-418">Si chiude ogni istruzione case con un'istruzione `break`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-418">You close each case statement with a `break` statement.</span></span> <span data-ttu-id="e1ac9-419">Se si dimentica di includere break in ogni blocco `case`, verrà eseguito anche il codice della successiva istruzione `case`. Un blocco di `switch` spesso ha un'istruzione `default` come ultimo caso per un &quot;tutto il resto&quot; opzione che viene eseguito se nessuno degli altri casi è true.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-419">(If you forget to include break in each `case` block, the code from the next `case` statement will run also.) A `switch` block often has a `default` statement as the last case for an &quot;everything else&quot; option that runs if none of the other cases are true.</span></span>

<span data-ttu-id="e1ac9-420">Risultato degli ultimi due blocchi condizionali visualizzati in un browser:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-420">The result of the last two conditional blocks displayed in a browser:</span></span>

![Razor-Img10](introducing-razor-syntax-c/_static/image10.jpg)

### <a name="looping-code"></a><span data-ttu-id="e1ac9-422">Codice ciclo</span><span class="sxs-lookup"><span data-stu-id="e1ac9-422">Looping Code</span></span>

<span data-ttu-id="e1ac9-423">Spesso è necessario eseguire ripetutamente le stesse istruzioni.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-423">You often need to run the same statements repeatedly.</span></span> <span data-ttu-id="e1ac9-424">A questo scopo, eseguire il ciclo.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-424">You do this by looping.</span></span> <span data-ttu-id="e1ac9-425">Ad esempio, si eseguono spesso le stesse istruzioni per ogni elemento in una raccolta di dati.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-425">For example, you often run the same statements for each item in a collection of data.</span></span> <span data-ttu-id="e1ac9-426">Se si conosce esattamente il numero di volte in cui si desidera eseguire il ciclo, è possibile utilizzare un ciclo `for`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-426">If you know exactly how many times you want to loop, you can use a `for` loop.</span></span> <span data-ttu-id="e1ac9-427">Questo tipo di ciclo è particolarmente utile per il conteggio e il conteggio:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-427">This kind of loop is especially useful for counting up or counting down:</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample53.html)]

<span data-ttu-id="e1ac9-428">Il ciclo inizia con la parola chiave `for`, seguita da tre istruzioni tra parentesi, ognuna terminata con un punto e virgola.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-428">The loop begins with the `for` keyword, followed by three statements in parentheses, each terminated with a semicolon.</span></span>

- <span data-ttu-id="e1ac9-429">All'interno delle parentesi, la prima istruzione (`var i=10;`) crea un contatore e lo inizializza su 10.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-429">Inside the parentheses, the first statement (`var i=10;`) creates a counter and initializes it to 10.</span></span> <span data-ttu-id="e1ac9-430">Non è necessario assegnare un nome al contatore &#8212; `i` è possibile usare qualsiasi variabile.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-430">You don't have to name the counter `i` &#8212; you can use any variable.</span></span> <span data-ttu-id="e1ac9-431">Quando viene eseguito il ciclo di `for`, il contatore viene incrementato automaticamente.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-431">When the `for` loop runs, the counter is automatically incremented.</span></span>
- <span data-ttu-id="e1ac9-432">La seconda istruzione (`i < 21;`) imposta la condizione per quanto riguarda la distanza da contare.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-432">The second statement (`i < 21;`) sets the condition for how far you want to count.</span></span> <span data-ttu-id="e1ac9-433">In questo caso, è necessario passare a un massimo di 20 (ovvero, continuare mentre il contatore è inferiore a 21).</span><span class="sxs-lookup"><span data-stu-id="e1ac9-433">In this case, you want it to go to a maximum of 20 (that is, keep going while the counter is less than 21).</span></span>
- <span data-ttu-id="e1ac9-434">La terza istruzione (`i++`) utilizza un operatore Increment, che specifica semplicemente che al contatore deve essere aggiunto 1 ogni volta che il ciclo viene eseguito.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-434">The third statement (`i++` ) uses an increment operator, which simply specifies that the counter should have 1 added to it each time the loop runs.</span></span>

<span data-ttu-id="e1ac9-435">All'interno delle parentesi graffe è riportato il codice che verrà eseguito per ogni iterazione del ciclo.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-435">Inside the braces is the code that will run for each iteration of the loop.</span></span> <span data-ttu-id="e1ac9-436">Il markup crea ogni volta un nuovo paragrafo (elemento`<p>`) e aggiunge una riga all'output, visualizzando il valore di `i` (il contatore).</span><span class="sxs-lookup"><span data-stu-id="e1ac9-436">The markup creates a new paragraph (`<p>` element) each time and adds a line to the output, displaying the value of `i` (the counter).</span></span> <span data-ttu-id="e1ac9-437">Quando si esegue questa pagina, l'esempio crea 11 righe che visualizzano l'output, con il testo in ogni riga che indica il numero dell'elemento.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-437">When you run this page, the example creates 11 lines displaying the output, with the text in each line indicating the item number.</span></span>

![Razor-img11](introducing-razor-syntax-c/_static/image11.jpg)

<span data-ttu-id="e1ac9-439">Se si lavora con una raccolta o una matrice, spesso si usa un ciclo `foreach`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-439">If you're working with a collection or array, you often use a `foreach` loop.</span></span> <span data-ttu-id="e1ac9-440">Una raccolta è un gruppo di oggetti simili e il ciclo di `foreach` consente di eseguire un'attività in ogni elemento della raccolta.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-440">A collection is a group of similar objects, and the `foreach` loop lets you carry out a task on each item in the collection.</span></span> <span data-ttu-id="e1ac9-441">Questo tipo di ciclo è utile per le raccolte, perché a differenza di un ciclo di `for`, non è necessario incrementare il contatore o impostare un limite.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-441">This type of loop is convenient for collections, because unlike a `for` loop, you don't have to increment the counter or set a limit.</span></span> <span data-ttu-id="e1ac9-442">Al contrario, il codice del ciclo `foreach` procede semplicemente nella raccolta fino a quando non viene completato.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-442">Instead, the `foreach` loop code simply proceeds through the collection until it's finished.</span></span>

<span data-ttu-id="e1ac9-443">Il codice seguente, ad esempio, restituisce gli elementi nella raccolta `Request.ServerVariables`, ovvero un oggetto che contiene informazioni sul server Web.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-443">For example, the following code returns the items in the `Request.ServerVariables` collection, which is an object that contains information about your web server.</span></span> <span data-ttu-id="e1ac9-444">Usa un ciclo `foreac` h per visualizzare il nome di ogni elemento creando un nuovo elemento `<li>` in un elenco puntato HTML.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-444">It uses a `foreac` h loop to display the name of each item by creating a new `<li>` element in an HTML bulleted list.</span></span>

[!code-html[Main](introducing-razor-syntax-c/samples/sample54.html)]

<span data-ttu-id="e1ac9-445">La parola chiave `foreach` è seguita da parentesi in cui viene dichiarata una variabile che rappresenta un singolo elemento nella raccolta (nell'esempio, `var item`), seguita dalla parola chiave `in`, seguita dalla raccolta in cui si desidera eseguire il ciclo.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-445">The `foreach` keyword is followed by parentheses where you declare a variable that represents a single item in the collection (in the example, `var item`), followed by the `in` keyword, followed by the collection you want to loop through.</span></span> <span data-ttu-id="e1ac9-446">Nel corpo del ciclo `foreach` è possibile accedere all'elemento corrente usando la variabile dichiarata in precedenza.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-446">In the body of the `foreach` loop, you can access the current item using the variable that you declared earlier.</span></span>

![Razor-Img12](introducing-razor-syntax-c/_static/image12.jpg)

<span data-ttu-id="e1ac9-448">Per creare un ciclo più generico, utilizzare l'istruzione `while`:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-448">To create a more general-purpose loop, use the `while` statement:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample55.cshtml)]

<span data-ttu-id="e1ac9-449">Un ciclo `while` inizia con la parola chiave `while`, seguito da parentesi in cui viene specificato il tempo di proseguimento del ciclo (in questo caso, purché `countNum` sia minore di 50), il blocco da ripetere.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-449">A `while` loop begins with the `while` keyword, followed by parentheses where you specify how long the loop continues (here, for as long as `countNum` is less than 50), then the block to repeat.</span></span> <span data-ttu-id="e1ac9-450">I cicli in genere incrementano (aggiungono) o decrementano (sottrae) una variabile o un oggetto usato per il conteggio.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-450">Loops typically increment (add to) or decrement (subtract from) a variable or object used for counting.</span></span> <span data-ttu-id="e1ac9-451">Nell'esempio, l'operatore `+=` aggiunge 1 a `countNum` ogni volta che il ciclo viene eseguito.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-451">In the example, the `+=` operator adds 1 to `countNum` each time the loop runs.</span></span> <span data-ttu-id="e1ac9-452">Per decrementare una variabile in un ciclo in cui viene conteggiato, usare l'operatore di decremento `-=`).</span><span class="sxs-lookup"><span data-stu-id="e1ac9-452">(To decrement a variable in a loop that counts down, you would use the decrement operator `-=`).</span></span>

## <a name="objects-and-collections"></a><span data-ttu-id="e1ac9-453">Oggetti e raccolte</span><span class="sxs-lookup"><span data-stu-id="e1ac9-453">Objects and Collections</span></span>

<span data-ttu-id="e1ac9-454">Quasi tutti gli elementi in un sito Web di ASP.NET sono un oggetto, inclusa la pagina Web stessa.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-454">Nearly everything in an ASP.NET website is an object, including the web page itself.</span></span> <span data-ttu-id="e1ac9-455">Questa sezione illustra alcuni oggetti importanti che verranno usati di frequente nel codice.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-455">This section discusses some important objects you'll work with frequently in your code.</span></span>

### <a name="page-objects"></a><span data-ttu-id="e1ac9-456">Oggetti Page</span><span class="sxs-lookup"><span data-stu-id="e1ac9-456">Page Objects</span></span>

<span data-ttu-id="e1ac9-457">L'oggetto più semplice in ASP.NET è la pagina.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-457">The most basic object in ASP.NET is the page.</span></span> <span data-ttu-id="e1ac9-458">È possibile accedere direttamente alle proprietà dell'oggetto pagina senza alcun oggetto idoneo.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-458">You can access properties of the page object directly without any qualifying object.</span></span> <span data-ttu-id="e1ac9-459">Il codice seguente ottiene il percorso del file della pagina utilizzando l'oggetto `Request` della pagina:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-459">The following code gets the page's file path, using the `Request` object of the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample56.cshtml)]

<span data-ttu-id="e1ac9-460">Per chiarire che si fa riferimento a proprietà e metodi nell'oggetto pagina corrente, è possibile usare facoltativamente la parola chiave `this` per rappresentare l'oggetto pagina nel codice.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-460">To make it clear that you're referencing properties and methods on the current page object, you can optionally use the keyword `this` to represent the page object in your code.</span></span> <span data-ttu-id="e1ac9-461">Di seguito è riportato l'esempio di codice precedente, con `this` aggiunto per rappresentare la pagina:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-461">Here is the previous code example, with `this` added to represent the page:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample57.cshtml)]

<span data-ttu-id="e1ac9-462">È possibile utilizzare le proprietà dell'oggetto `Page` per ottenere una grande quantità di informazioni, ad esempio:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-462">You can use properties of the `Page` object to get a lot of information, such as:</span></span>

- <span data-ttu-id="e1ac9-463">`Request`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-463">`Request`.</span></span> <span data-ttu-id="e1ac9-464">Come si è già visto, si tratta di una raccolta di informazioni sulla richiesta corrente, tra cui il tipo di browser che ha effettuato la richiesta, l'URL della pagina, l'identità dell'utente e così via.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-464">As you've already seen, this is a collection of information about the current request, including what type of browser made the request, the URL of the page, the user identity, etc.</span></span>
- <span data-ttu-id="e1ac9-465">`Response`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-465">`Response`.</span></span> <span data-ttu-id="e1ac9-466">Si tratta di una raccolta di informazioni sulla risposta (pagina) che verrà inviata al browser al termine dell'esecuzione del codice server.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-466">This is a collection of information about the response (page) that will be sent to the browser when the server code has finished running.</span></span> <span data-ttu-id="e1ac9-467">Ad esempio, è possibile usare questa proprietà per scrivere informazioni nella risposta.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-467">For example, you can use this property to write information into the response.</span></span> 

    [!code-cshtml[Main](introducing-razor-syntax-c/samples/sample58.cshtml)]

<a id="ID_CollectionsAndObjects"></a>
### <a name="collection-objects-arrays-and-dictionaries"></a><span data-ttu-id="e1ac9-468">Oggetti Collection (matrici e dizionari)</span><span class="sxs-lookup"><span data-stu-id="e1ac9-468">Collection Objects (Arrays and Dictionaries)</span></span>

<span data-ttu-id="e1ac9-469">Una *raccolta* è un gruppo di oggetti dello stesso tipo, ad esempio una raccolta di oggetti `Customer` di un database.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-469">A *collection* is a group of objects of the same type, such as a collection of `Customer` objects from a database.</span></span> <span data-ttu-id="e1ac9-470">ASP.NET contiene molte raccolte predefinite, ad esempio la raccolta `Request.Files`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-470">ASP.NET contains many built-in collections, like the `Request.Files` collection.</span></span>

<span data-ttu-id="e1ac9-471">Spesso si utilizzano i dati nelle raccolte.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-471">You'll often work with data in collections.</span></span> <span data-ttu-id="e1ac9-472">Due tipi di raccolta comuni sono la *matrice* e il *dizionario*.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-472">Two common collection types are the *array* and the *dictionary*.</span></span> <span data-ttu-id="e1ac9-473">Una matrice è utile quando si vuole archiviare una raccolta di elementi simili, ma non si vuole creare una variabile separata per contenere ogni elemento:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-473">An array is useful when you want to store a collection of similar items but don't want to create a separate variable to hold each item:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample59.cshtml)]

<span data-ttu-id="e1ac9-474">Con le matrici si dichiara un tipo di dati specifico, ad esempio `string`, `int`o `DateTime`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-474">With arrays, you declare a specific data type, such as `string`, `int`, or `DateTime`.</span></span> <span data-ttu-id="e1ac9-475">Per indicare che la variabile può contenere una matrice, è necessario aggiungere parentesi quadre alla dichiarazione, ad esempio `string[]` o `int[]`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-475">To indicate that the variable can contain an array, you add brackets to the declaration (such as `string[]` or `int[]`).</span></span> <span data-ttu-id="e1ac9-476">È possibile accedere agli elementi di una matrice usando la relativa posizione (indice) o usando l'istruzione `foreach`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-476">You can access items in an array using their position (index) or by using the `foreach` statement.</span></span> <span data-ttu-id="e1ac9-477">Gli indici di matrice sono &#8212; in base zero, ovvero il primo elemento si trova nella posizione 0, il secondo elemento si trova nella posizione 1 e così via.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-477">Array indexes are zero-based &#8212; that is, the first item is at position 0, the second item is at position 1, and so on.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample60.cshtml)]

<span data-ttu-id="e1ac9-478">È possibile determinare il numero di elementi in una matrice ottenendone la proprietà `Length`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-478">You can determine the number of items in an array by getting its `Length` property.</span></span> <span data-ttu-id="e1ac9-479">Per ottenere la posizione di un elemento specifico nella matrice (per eseguire la ricerca nella matrice), usare il metodo `Array.IndexOf`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-479">To get the position of a specific item in the array (to search the array), use the `Array.IndexOf` method.</span></span> <span data-ttu-id="e1ac9-480">È anche possibile eseguire operazioni come invertire il contenuto di una matrice (il metodo `Array.Reverse`) o ordinare il contenuto (metodo `Array.Sort`).</span><span class="sxs-lookup"><span data-stu-id="e1ac9-480">You can also do things like reverse the contents of an array (the `Array.Reverse` method) or sort the contents (the `Array.Sort` method).</span></span>

<span data-ttu-id="e1ac9-481">L'output del codice della matrice di stringhe visualizzato in un browser:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-481">The output of the string array code displayed in a browser:</span></span>

![Razor-Img13](introducing-razor-syntax-c/_static/image13.jpg)

<span data-ttu-id="e1ac9-483">Un dizionario è una raccolta di coppie chiave/valore, in cui è possibile specificare la chiave (o il nome) per impostare o recuperare il valore corrispondente:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-483">A dictionary is a collection of key/value pairs, where you provide the key (or name) to set or retrieve the corresponding value:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample61.cshtml)]

<span data-ttu-id="e1ac9-484">Per creare un dizionario, usare la parola chiave `new` per indicare che si sta creando un nuovo oggetto Dictionary.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-484">To create a dictionary, you use the `new` keyword to indicate that you're creating a new dictionary object.</span></span> <span data-ttu-id="e1ac9-485">È possibile assegnare un dizionario a una variabile usando la parola chiave `var`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-485">You can assign a dictionary to a variable using the `var` keyword.</span></span> <span data-ttu-id="e1ac9-486">È possibile indicare i tipi di dati degli elementi nel dizionario usando le parentesi angolari (`< >`).</span><span class="sxs-lookup"><span data-stu-id="e1ac9-486">You indicate the data types of the items in the dictionary using angle brackets ( `< >` ).</span></span> <span data-ttu-id="e1ac9-487">Alla fine della dichiarazione, è necessario aggiungere una coppia di parentesi, perché si tratta in realtà di un metodo che crea un nuovo dizionario.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-487">At the end of the declaration, you must add a pair of parentheses, because this is actually a method that creates a new dictionary.</span></span>

<span data-ttu-id="e1ac9-488">Per aggiungere elementi al dizionario, è possibile chiamare il metodo `Add` della variabile Dictionary (in questo caso`myScores`), quindi specificare una chiave e un valore.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-488">To add items to the dictionary, you can call the `Add` method of the dictionary variable (`myScores` in this case), and then specify a key and a value.</span></span> <span data-ttu-id="e1ac9-489">In alternativa, è possibile usare le parentesi quadre per indicare la chiave ed eseguire una semplice assegnazione, come nell'esempio seguente:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-489">Alternatively, you can use square brackets to indicate the key and do a simple assignment, as in the following example:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample62.cs)]

<span data-ttu-id="e1ac9-490">Per ottenere un valore dal dizionario, è necessario specificare la chiave tra parentesi quadre:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-490">To get a value from the dictionary, you specify the key in brackets:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample63.cs)]

## <a name="calling-methods-with-parameters"></a><span data-ttu-id="e1ac9-491">Chiamata di metodi con parametri</span><span class="sxs-lookup"><span data-stu-id="e1ac9-491">Calling Methods with Parameters</span></span>

<span data-ttu-id="e1ac9-492">Come è stato letto in precedenza in questo articolo, gli oggetti con cui si programma con possono avere metodi.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-492">As you read earlier in this article, the objects that you program with can have methods.</span></span> <span data-ttu-id="e1ac9-493">Ad esempio, un oggetto `Database` potrebbe avere un metodo `Database.Connect`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-493">For example, a `Database` object might have a `Database.Connect` method.</span></span> <span data-ttu-id="e1ac9-494">Molti metodi dispongono anche di uno o più parametri.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-494">Many methods also have one or more parameters.</span></span> <span data-ttu-id="e1ac9-495">Un *parametro* è un valore che viene passato a un metodo per consentire al metodo di completare l'attività.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-495">A *parameter* is a value that you pass to a method to enable the method to complete its task.</span></span> <span data-ttu-id="e1ac9-496">Si osservi, ad esempio, una dichiarazione per il metodo `Request.MapPath`, che accetta tre parametri:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-496">For example, look at a declaration for the `Request.MapPath` method, which takes three parameters:</span></span>

[!code-csharp[Main](introducing-razor-syntax-c/samples/sample64.cs)]

<span data-ttu-id="e1ac9-497">(La riga è stata incapsulata per renderla più leggibile.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-497">(The line has been wrapped to make it more readable.</span></span> <span data-ttu-id="e1ac9-498">Tenere presente che è possibile inserire interruzioni di riga praticamente in qualsiasi punto, tranne che all'interno di stringhe racchiuse tra virgolette.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-498">Remember that you can put line breaks almost any place except inside strings that are enclosed in quotation marks.)</span></span>

<span data-ttu-id="e1ac9-499">Questo metodo restituisce il percorso fisico sul server che corrisponde a un percorso virtuale specificato.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-499">This method returns the physical path on the server that corresponds to a specified virtual path.</span></span> <span data-ttu-id="e1ac9-500">I tre parametri per il metodo sono `virtualPath`, `baseVirtualDir`e `allowCrossAppMapping`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-500">The three parameters for the method are `virtualPath`, `baseVirtualDir`, and `allowCrossAppMapping`.</span></span> <span data-ttu-id="e1ac9-501">Si noti che nella dichiarazione i parametri sono elencati con i tipi di dati che verranno accettati. Quando si chiama questo metodo, è necessario fornire valori per tutti e tre i parametri.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-501">(Notice that in the declaration, the parameters are listed with the data types of the data that they'll accept.) When you call this method, you must supply values for all three parameters.</span></span>

<span data-ttu-id="e1ac9-502">Il sintassi Razor offre due opzioni per il passaggio di parametri a un metodo: *parametri posizionali* e *parametri denominati*.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-502">The Razor syntax gives you two options for passing parameters to a method: *positional parameters* and *named parameters*.</span></span> <span data-ttu-id="e1ac9-503">Per chiamare un metodo usando parametri posizionali, passare i parametri in un ordine rigoroso specificato nella dichiarazione di metodo.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-503">To call a method using positional parameters, you pass the parameters in a strict order that's specified in the method declaration.</span></span> <span data-ttu-id="e1ac9-504">In genere si conosce questo ordine leggendo la documentazione per il metodo. È necessario seguire l'ordine e non è possibile ignorare i parametri &#8212; , se necessario, passare una stringa vuota (`""`) o `null` per un parametro posizionale per il quale non si dispone di un valore.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-504">(You would typically know this order by reading documentation for the method.) You must follow the order, and you can't skip any of the parameters &#8212; if necessary, you pass an empty string (`""`) or `null` for a positional parameter that you don't have a value for.</span></span>

<span data-ttu-id="e1ac9-505">Nell'esempio seguente si presuppone che sia presente una cartella denominata *Scripts* nel sito Web.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-505">The following example assumes you have a folder named *scripts* on your website.</span></span> <span data-ttu-id="e1ac9-506">Il codice chiama il metodo `Request.MapPath` e passa i valori per i tre parametri nell'ordine corretto.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-506">The code calls the `Request.MapPath` method and passes values for the three parameters in the correct order.</span></span> <span data-ttu-id="e1ac9-507">Viene quindi visualizzato il percorso mappato risultante.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-507">It then displays the resulting mapped path.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample65.cshtml)]

<span data-ttu-id="e1ac9-508">Quando un metodo presenta molti parametri, è possibile rendere il codice più leggibile usando i parametri denominati.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-508">When a method has many parameters, you can keep your code more readable by using named parameters.</span></span> <span data-ttu-id="e1ac9-509">Per chiamare un metodo utilizzando parametri denominati, è necessario specificare il nome del parametro seguito da due punti (:), quindi il valore.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-509">To call a method using named parameters, you specify the parameter name followed by a colon (:), and then the value.</span></span> <span data-ttu-id="e1ac9-510">Il vantaggio dei parametri denominati è che è possibile passarli in qualsiasi ordine.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-510">The advantage of named parameters is that you can pass them in any order you want.</span></span> <span data-ttu-id="e1ac9-511">Uno svantaggio è che la chiamata al metodo non è compatta.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-511">(A disadvantage is that the method call is not as compact.)</span></span>

<span data-ttu-id="e1ac9-512">Nell'esempio seguente viene chiamato lo stesso metodo precedente, ma vengono utilizzati i parametri denominati per fornire i valori:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-512">The following example calls the same method as above, but uses named parameters to supply the values:</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample66.cshtml)]

<span data-ttu-id="e1ac9-513">Come si può notare, i parametri vengono passati in un ordine diverso.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-513">As you can see, the parameters are passed in a different order.</span></span> <span data-ttu-id="e1ac9-514">Tuttavia, se si esegue l'esempio precedente e questo esempio, restituirà lo stesso valore.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-514">However, if you run the previous example and this example, they'll return the same value.</span></span>

<a id="ID_HandlingErrors"></a>
## <a name="handling-errors"></a><span data-ttu-id="e1ac9-515">Gestione degli errori</span><span class="sxs-lookup"><span data-stu-id="e1ac9-515">Handling Errors</span></span>

### <a name="try-catch-statements"></a><span data-ttu-id="e1ac9-516">Istruzioni try-catch</span><span class="sxs-lookup"><span data-stu-id="e1ac9-516">Try-Catch Statements</span></span>

<span data-ttu-id="e1ac9-517">Nel codice saranno spesso presenti istruzioni che potrebbero avere esito negativo per motivi esterni al controllo.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-517">You'll often have statements in your code that might fail for reasons outside your control.</span></span> <span data-ttu-id="e1ac9-518">Ad esempio:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-518">For example:</span></span>

- <span data-ttu-id="e1ac9-519">Se il codice tenta di creare o accedere a un file, è possibile che si verifichino errori di questo tipo.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-519">If your code tries to create or access a file, all sorts of errors might occur.</span></span> <span data-ttu-id="e1ac9-520">Il file desiderato potrebbe non esistere, potrebbe essere bloccato, il codice potrebbe non avere le autorizzazioni e così via.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-520">The file you want might not exist, it might be locked, the code might not have permissions, and so on.</span></span>
- <span data-ttu-id="e1ac9-521">Analogamente, se il codice tenta di aggiornare i record in un database, potrebbero verificarsi problemi relativi alle autorizzazioni, la connessione al database potrebbe essere eliminata, i dati da salvare potrebbero non essere validi e così via.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-521">Similarly, if your code tries to update records in a database, there can be permissions issues, the connection to the database might be dropped, the data to save might be invalid, and so on.</span></span>

<span data-ttu-id="e1ac9-522">In termini di programmazione, queste situazioni sono denominate *eccezioni*.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-522">In programming terms, these situations are called *exceptions*.</span></span> <span data-ttu-id="e1ac9-523">Se il codice rileva un'eccezione, genera (genera) un messaggio di errore che, al meglio, fastidioso per gli utenti:</span><span class="sxs-lookup"><span data-stu-id="e1ac9-523">If your code encounters an exception, it generates (throws) an error message that's, at best, annoying to users:</span></span>

![Razor-Img14](introducing-razor-syntax-c/_static/image14.jpg)

<span data-ttu-id="e1ac9-525">Nelle situazioni in cui il codice potrebbe rilevare eccezioni e, per evitare messaggi di errore di questo tipo, è possibile usare le istruzioni `try/catch`.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-525">In situations where your code might encounter exceptions, and in order to avoid error messages of this type, you can use `try/catch` statements.</span></span> <span data-ttu-id="e1ac9-526">Nell'istruzione `try` viene eseguito il codice che si sta controllando.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-526">In the `try` statement, you run the code that you're checking.</span></span> <span data-ttu-id="e1ac9-527">In una o più istruzioni `catch` è possibile cercare errori specifici (tipi specifici di eccezioni) che potrebbero essersi verificati.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-527">In one or more `catch` statements, you can look for specific errors (specific types of exceptions) that might have occurred.</span></span> <span data-ttu-id="e1ac9-528">È possibile includere il numero di istruzioni `catch` che è necessario cercare per individuare gli errori previsti.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-528">You can include as many `catch` statements as you need to look for errors that you are anticipating.</span></span>

> [!NOTE]
> <span data-ttu-id="e1ac9-529">È consigliabile evitare di usare il metodo `Response.Redirect` nelle istruzioni `try/catch`, perché può causare un'eccezione nella pagina.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-529">We recommend that you avoid using the `Response.Redirect` method in `try/catch` statements, because it can cause an exception in your page.</span></span>

<span data-ttu-id="e1ac9-530">Nell'esempio seguente viene illustrata una pagina che crea un file di testo alla prima richiesta e quindi Visualizza un pulsante che consente all'utente di aprire il file.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-530">The following example shows a page that creates a text file on the first request and then displays a button that lets the user open the file.</span></span> <span data-ttu-id="e1ac9-531">Nell'esempio viene utilizzato intenzionalmente un nome di file non valido, in modo che venga generata un'eccezione.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-531">The example deliberately uses a bad file name so that it will cause an exception.</span></span> <span data-ttu-id="e1ac9-532">Il codice include `catch` istruzioni per due possibili eccezioni: `FileNotFoundException`, che si verifica se il nome del file non è valido e `DirectoryNotFoundException`, che si verifica se ASP.NET non riesce a trovare la cartella.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-532">The code includes `catch` statements for two possible exceptions: `FileNotFoundException`, which occurs if the file name is bad, and `DirectoryNotFoundException`, which occurs if ASP.NET can't even find the folder.</span></span> <span data-ttu-id="e1ac9-533">È possibile rimuovere il commento da un'istruzione nell'esempio per verificarne l'esecuzione quando tutto funziona correttamente.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-533">(You can uncomment a statement in the example in order to see how it runs when everything works properly.)</span></span>

<span data-ttu-id="e1ac9-534">Se il codice non ha gestito l'eccezione, verrà visualizzata una pagina di errore simile alla schermata precedente.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-534">If your code didn't handle the exception, you would see an error page like the previous screen shot.</span></span> <span data-ttu-id="e1ac9-535">Tuttavia, la sezione `try/catch` consente di impedire all'utente di visualizzare questi tipi di errori.</span><span class="sxs-lookup"><span data-stu-id="e1ac9-535">However, the `try/catch` section helps prevent the user from seeing these types of errors.</span></span>

[!code-cshtml[Main](introducing-razor-syntax-c/samples/sample67.cshtml)]

## <a name="additional-resources"></a><span data-ttu-id="e1ac9-536">Risorse aggiuntive</span><span class="sxs-lookup"><span data-stu-id="e1ac9-536">Additional Resources</span></span>

<span data-ttu-id="e1ac9-537">**Programmazione con Visual Basic**</span><span class="sxs-lookup"><span data-stu-id="e1ac9-537">**Programming with Visual Basic**</span></span>

[<span data-ttu-id="e1ac9-538">Appendice: sintassi e linguaggio Visual Basic</span><span class="sxs-lookup"><span data-stu-id="e1ac9-538">Appendix: Visual Basic Language and Syntax</span></span>](https://go.microsoft.com/fwlink/?LinkId=202908)

<span data-ttu-id="e1ac9-539">**Documentazione di riferimento**</span><span class="sxs-lookup"><span data-stu-id="e1ac9-539">**Reference Documentation**</span></span>

[<span data-ttu-id="e1ac9-540">ASP.NET 2.0</span><span class="sxs-lookup"><span data-stu-id="e1ac9-540">ASP.NET</span></span>](https://msdn.microsoft.com/library/ee532866.aspx)

[<span data-ttu-id="e1ac9-541">C#Linguaggio</span><span class="sxs-lookup"><span data-stu-id="e1ac9-541">C# Language</span></span>](https://msdn.microsoft.com/library/kx37x362.aspx)
