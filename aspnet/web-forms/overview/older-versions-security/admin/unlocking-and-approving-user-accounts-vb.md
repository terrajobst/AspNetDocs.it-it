...
[Scarica codice](https://download.microsoft.com/download/6/0/e/60e1bd94-e5f9-4d5a-a079-f23c98f4f67d/VB.14.zip) o [Scarica PDF](https://download.microsoft.com/download/6/0/e/60e1bd94-e5f9-4d5a-a079-f23c98f4f67d/aspnet_tutorial14_UnlockAndApprove_vb.pdf)
...
> [!NOTE]
> Se è stato scaricato il codice per <a id="Tutorial13"> </a>l'esercitazione sul [*ripristino e la modifica delle password*](recovering-and-changing-passwords-vb.md) , è possibile notare che la pagina `ManageUsers.aspx` contiene già un set di collegamenti "Gestisci" e la pagina `UserInformation.aspx` fornisce un'interfaccia per la modifica della password dell'utente selezionato. Ho deciso di non replicare questa funzionalità nel codice associato a questa esercitazione, perché funzionava aggirando l'API di appartenenza e agendo direttamente con il database di SQL Server per modificare la password di un utente. Questa esercitazione inizia da zero con la pagina `UserInformation.aspx`.
...
[![HyperLinkField aggiunge un](unlocking-and-approving-user-accounts-vb/_static/image2.png)](unlocking-and-approving-user-accounts-vb/_static/image1.png)

**Figura 1**: HyperLinkField aggiunge un collegamento "Gestisci" per ogni account utente ([fare clic per visualizzare l'immagine con dimensioni complete](unlocking-and-approving-user-accounts-vb/_static/image3.png))
...
**Figura 2**: creare l'interfaccia utente per `UserInformation.aspx` ([fare clic per visualizzare l'immagine con dimensioni complete](unlocking-and-approving-user-accounts-vb/_static/image6.png))
...
[![Chris non è stato approvato](unlocking-and-approving-user-accounts-vb/_static/image8.png)](unlocking-and-approving-user-accounts-vb/_static/image7.png)

**Figura 3**: Chris è stato non approvato ([fare clic per visualizzare l'immagine con dimensioni complete](unlocking-and-approving-user-accounts-vb/_static/image9.png))
...
[Non è possibile eseguire l'accesso ![Chris perché il suo account non è approvato](unlocking-and-approving-user-accounts-vb/_static/image11.png)](unlocking-and-approving-user-accounts-vb/_static/image10.png)

**Figura 4**: Chris non può eseguire l'accesso perché il suo account non è approvato ([fare clic per visualizzare l'immagine con dimensioni complete](unlocking-and-approving-user-accounts-vb/_static/image12.png))
...
**Figura 5**: Dave è stato bloccato dal sistema ([fare clic per visualizzare l'immagine con dimensioni complete](unlocking-and-approving-user-accounts-vb/_static/image15.png))
...
Impostare la proprietà `BodyFileName` del `MailDefinition`su "~/EmailTemplates/CreateUserWizard.txt" e la relativa proprietà `Subject` su "Benvenuti nel sito Web. Attiva l'account ".
...
**Figura 6**: il nuovo utente riceve un messaggio di posta elettronica con un collegamento all'URL di verifica ([fare clic per visualizzare l'immagine con dimensioni complete](unlocking-and-approving-user-accounts-vb/_static/image18.png))
...
La maggior parte del codice precedente verifica che l'ID utente fornito tramite QueryString esista, che si tratta di un valore `Guid` valido e che faccia riferimento a un account utente esistente. Se vengono superati tutti questi controlli, l'account utente viene approvato; in caso contrario, viene visualizzato un messaggio di stato appropriato.

Nella figura 7 viene illustrata la pagina `Verification.aspx` se visitata tramite un browser.

[![l'account del nuovo utente è ora approvato](unlocking-and-approving-user-accounts-vb/_static/image20.png)](unlocking-and-approving-user-accounts-vb/_static/image19.png)

**Figura 7**: l'account del nuovo utente è ora approvato ([fare clic per visualizzare l'immagine con dimensioni complete](unlocking-and-approving-user-accounts-vb/_static/image21.png))
