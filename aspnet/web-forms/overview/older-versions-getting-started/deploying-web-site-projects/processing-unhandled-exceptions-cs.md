...
[Visualizzare o scaricare il codice di esempio](https://github.com/dotnet/AspNetDocs/tree/master/aspnet/web-forms/overview/older-versions-getting-started/deploying-web-site-projects/processing-unhandled-exceptions-cs/samples) ([procedura per il download](/aspnet/core/tutorials/index#how-to-download-a-sample))
...
([Fare clic per visualizzare l'immagine con dimensioni complete](processing-unhandled-exceptions-cs/_static/image3.png))

Il contenuto e la struttura del file di `Global.asax` creato da Visual Studio differiscono leggermente a seconda che si usi un progetto di applicazione Web (WAP) o un progetto di sito Web (WSP). Con un WAP, il `Global.asax` viene implementato come due file distinti `Global.asax` e `Global.asax.cs`. Il file di `Global.asax` non contiene elementi, ma una direttiva `@Application` che fa riferimento al file di `.cs`; i gestori di eventi di interesse vengono definiti nel file di `Global.asax.cs`. Per WSPs, viene creato un solo file, `Global.asax`e i gestori eventi sono definiti in un blocco di `<script runat="server">`.
...
> [!NOTE]
> Quando si distribuisce l'applicazione ASP.NET, è necessario copiare il file di `Global.asax` nell'ambiente di produzione. Il file di `Global.asax.cs`, creato nel WAP, non deve essere copiato in produzione perché questo codice viene compilato nell'assembly del progetto.
...
([Fare clic per visualizzare l'immagine con dimensioni complete](processing-unhandled-exceptions-cs/_static/image6.png))

[![](processing-unhandled-exceptions-cs/_static/image8.png)](processing-unhandled-exceptions-cs/_static/image7.png)

**Figura 3**: la notifica tramite posta elettronica include i dettagli dell'eccezione YSOD come allegato  
([Fare clic per visualizzare l'immagine con dimensioni complete](processing-unhandled-exceptions-cs/_static/image9.png))
...
([Fare clic per visualizzare l'immagine con dimensioni complete](processing-unhandled-exceptions-cs/_static/image12.png))
