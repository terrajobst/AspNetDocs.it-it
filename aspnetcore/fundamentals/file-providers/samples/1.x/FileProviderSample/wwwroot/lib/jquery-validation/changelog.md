...
---
<a name="1160--2016-12-01"></a>1.16.0 / 01-12-2016
==================

## <a name="additional"></a>Oggetti
  * Ottimizzazione degli algoritmi cifES e nieES. Chiude #1826

## <a name="build"></a>Compilazione
  * Inclusione della versione minimizzata nel pacchetto NPM
  * Eliminazione delle dipendenze di sviluppo nelle versioni più recenti

## <a name="core"></a>Base
  * Aggiunta di un'associazione per l'input con tipo pulsante. Chiude #1891
  * Supporto di jquery3. Chiude #1866
  * Modifica dell'alias jQuery 'expr[":"]' in 'expr.pseudos'

## <a name="localisation"></a>Localizzazione
  * Aggiunta della traduzione Urdu. Chiude #1873.

## <a name="localization"></a>Localizzazione
  * Correzione dell'estensione di file non corretta per la traduzione az. Chiude #1890.
  * Aggiunta di una traduzione mancante in pt-BR (chiude #1897)
  * Correzione di un errore ortografico in un file di lingua araba.

## <a name="tests"></a>Test
  * Aggiornamento di QUnit a 2.0.

## <a name="umd"></a>UMD
  * Supporto migliorato per CommonJS.

<a name="1151--2016-07-22"></a>1.15.1 / 22-07-2016
==================

## <a name="additional"></a>Oggetti
  * Correzione della convalida multipla di tipo mime
  * Per l'IBAN sono richiesti almeno 5 caratteri (chiude #1797, corregge #1674)
  * Correzione del riferimento jQuery notEqualTo

## <a name="core"></a>Base
  * Aggiunta di test non riuscito per #1805
  * Correzione della convalida di gruppi con 3 e più campi
  * Correzione delle regressioni introdotte in 1644 # e #1657 (chiude #1800)
  * Aggiornamento del passaggio di convalida per gestire correttamente i punti a virgola mobile
  * Correzione dell'errore quando si chiama $.fn.rules() per un elemento esterno al form
  * Chiamata di `errorPlacement` nell'ambito del controllo server di convalida
  * Correzione di un problema con gli elementi contenteditable nei form, a causa del quale gli eventi per la convalida di input singolo causano eccezioni

## <a name="demo"></a>Demo
  * Aggiunta di collegamenti alle demo di Bootstrap e Semantic-UI a index.html
  * Usare `.on()` anziché `.validateDelegate()`

## <a name="localization"></a>Localizzazione
  * Aggiunta della lingua azero

## <a name="tests"></a>Test
  * Aggiunta di unit test di regressione per PR #1760

<a name="1150--2016-02-24"></a>1.15.0 / 24-02-2016
==================

## <a name="all"></a>Tutti
  * Correzione di problemi di stile del codice

## <a name="core"></a>Base
  * `resetForm` deve rimuovere anche la classe `valid` dagli elementi.
  * Annullamento dell'evidenziazione di un campo se è già evidenziato quando si usa la regola remota.
  * Associare l'evento `blur` una sola volta nella regola `equalTo`
  * Correzione di un errore durante la chiamata di .rules() su un set jquery vuoto.
  * Correzione della gestione dei messaggi di errore con i gruppi di input.
  * Correzione di TypeError in `showLabel` quando si usano le impostazioni `groups`
  * Aggiunta di un modo per passare il nome di metodo in remoto
  * L'attivazione della convalida non riesce quando il campo successivo è già compilato (corregge #1508)
  * La regola obbligatoria ha precedenza rispetto alle regole di numeri e cifre
  * Errore nascosto ma la classe degli errori di input non viene rimossa
  * La convalida remota usa messaggi di errore errati
  * Evidenziazione fissa dei campi con la convalida remota.
  * Correzione del selettore `:filled` per più elementi di selezione.
  * Aggiunta di un riferimento nella documentazione a jQuery.validator.methods
  * Spostamento dell'elaborazione dei messaggi da `formatAndAdd` a `defaultMessage`
  * ErrorList deve contenere solo gli errori previsti
  * Estrazione del nome del file senza includere "C:\fakepath\"
  * Supporto dell'attributo step HTML5. Corregge #1295
  * Aggiunta del supporto per la classe "pending" per le richieste in sospeso
  * Aggiunta del normalizzatore (#1602)
  * Suddivisione del metodo `creditcard`
  * Escape di errorID per l'uso nell'espressione regolare, non per compilare aria-describedby
  * Escape delle virgolette singole nei nomi per evitare un errore di Sizzle
  * Invece di usare il valore del campo di convalida per ignorare la chiamata API, usare l'oggetto dati serializzato della richiesta
  * Aggiunta del supporto per i tag contentEditable

## <a name="additional"></a>Oggetti
  * BIC: cifre da 1 a 9 consentite nella seconda posizione del percorso
  * Escape corretto per l'espressione regolare del metodo accept.
  * Controllo senza distinzione tra maiuscole e minuscole per BIC
  * Correzione di postalCodeCA per escludere le combinazioni non valide
  * Maggiore flessibilità per il metodo postalCodeCA

## <a name="localization"></a>Localizzazione
  * Aggiunta la localizzazione in macedone.
  * Aggiunta di messaggio di modello mancante in polacco (adamwojtkiewicz)
  * Correzione della traduzione in persiano del messaggio di min/max.
  * Aggiornamento di messages_sk.js
  * Aggiornamento della traduzione in malese
  * Messaggi inclusi da metodi aggiuntivi
  * Miglioramento della traduzione pt_BR e correzione di un errore ortografico nella chiave 'cifES'.
